...
namespace succinct {
    ...
    public:
        ...
        inline void append_bits(uint128_t bits, size_t len) {
            // check there are no spurious bits
            assert(len == 128 || (bits >> len) == 0);
            uint64_t hi, lo;
            hi = uint64_t(bits >> 64);
            lo = uint64_t(bits);
            if (len > 64) {
                append_bits(hi, len - 64);
                append_bits(lo, 64);
            } else {
                append_bits(lo, len);
            }
    ...
    public:
        ...
        struct enumerator {
            ...
            /*scritta da noi*/
            inline uint128_t take128(size_t l) {
                //assert(l > 0);
                if (l > 64) {
                    uint128_t hi, lo;
                    hi = take(l - 64);
            ...
            inline void move(size_t new_pos) {
                m_pos = new_pos;
                m_buf = 0;
                m_avail = 0;
                m_bv->data().prefetch(m_pos / 64);
