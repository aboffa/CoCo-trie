...
namespace succinct {
    namespace broadword {
        ...
        static const uint64_t incr_step_8 =
                0x80ULL << 56 | 0x40ULL << 48 | 0x20ULL << 40 | 0x10ULL << 32 | 0x8ULL << 24 | 0x4ULL << 16 |
                0x2ULL << 8 | 0x1;
        ...
        inline uint8_t msb(uint128_t x, unsigned long &ret) {
            return intrinsics::bsr128(&ret, x);
        }

#if defined(__clang__)
        inline uint8_t msb(uint256_t x, unsigned long &ret) {
            return intrinsics::bsr256(&ret, x);
        }
#endif
        inline uint8_t msb(uint128_t x) {
            assert(x);
            unsigned long ret = -1U;
            msb(x, ret);
            return (uint8_t) ret;
...
#if defined(__clang__)
        inline uint8_t msb(uint256_t x) {
            assert(x);
            unsigned long ret = -1U;
            msb(x, ret);
            return (uint8_t) ret;
